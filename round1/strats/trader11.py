...
# ASH: SMA(window=1000 ticks) fair value (static 10000 for first 10k timestamps) + take/clear/make | IPR: buy-and-hold
...
class SMATrader(ProductTrader):
    def __init__(self, state, new_traderData):
        super().__init__("ASH_COATED_OSMIUM", state, new_traderData)
        self.sma_window = PARAMS[self.name]["sma_window"]
        self.fair_value = self.compute_fair_value()

    def compute_fair_value(self):
        """
        Simple Moving Average of mid-price over a rolling window.
        For the first 10,000 timestamps (100 ticks), uses static FV=10000.
        Always collects mid-prices from tick 0 so the buffer is ready.
        """
        mid = self.compute_mid_price()

        # retrieve price history from last traderData
        price_history = self.last_traderData[self.name].get("price_history", [])

        # append current mid if available
        if mid is not None:
            price_history.append(mid)

        # keep only last sma_window entries
        price_history = price_history[-self.sma_window:]

        # persist to new traderData
        self.new_traderData[self.name]["price_history"] = price_history

        # for first 10,000 timestamps, use static fair value
        if self.timestamp < 10000:
            return 10000

        # compute SMA
        if len(price_history) > 0:
            return round(sum(price_history) / len(price_history))
...
class Trader:
    def run(self, state: TradingState):
        ...
        sma_trader = SMATrader(state, new_traderData)
        lin_trend_trader = LinearTrendTrader(state, new_traderData)

        result.update(sma_trader.get_orders())
        result.update(lin_trend_trader.get_orders())

        sma_trader.update_traderData()
        lin_trend_trader.update_traderData()

        new_traderData.update(sma_trader.new_traderData)
