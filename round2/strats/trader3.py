...
# ASH: static_fair_value + take/make
# IPR: buy-and-hold
...
class MeanReversionTrader(ProductTrader):
    ...
    def get_orders(self):
        ...
        # STEP 2: Clear Orders
        if self.expected_position > 0.7 * self.position_limit:
            clear_sv = self.expected_position * 0.333
            self.sell(self.fair_value, clear_sv)
        
        elif self.expected_position < -0.7 * self.position_limit:
            clear_bv = self.expected_position * 0.333
            self.buy(self.fair_value, clear_bv)
