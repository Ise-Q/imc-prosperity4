...
# Expiry-aware positioning:
# VEV_5000: deeply ITM (+232 ticks), underpriced vs BS → BUY & HOLD aggressively
# VEV_5100: ITM (+132 ticks), underpriced by 5.7 ticks vs BS → BUY & HOLD
# VEV_5200: near ATM (+32 ticks), risky → neutral MM only
# VEV_5300: OTM (-68 ticks), overpriced → SELL & HOLD (keep premium at expiry)
# VEV_5400: deep OTM → neutral MM only
SELL_BIAS = {"VEV_5300"}          # build net short → keep premium if VEF<5300
BUY_BIAS  = {"VEV_5000","VEV_5100"}  # build net long → collect intrinsic at expiry
...
class VEVTrader(ProductTrader):
    ...
    def get_orders(self):
        ...
        # For deep ITM buy positions: sweep asks aggressively up to BS fair
        # For OTM sell positions: sweep bids aggressively down to BS fair
        is_sell_bias = self.name in SELL_BIAS
        is_buy_bias  = self.name in BUY_BIAS

        if is_buy_bias and self.od.sell_orders:
            # Sweep all asks up to fair value — buy deeply ITM options
            for sp in sorted(self.od.sell_orders.keys()):
                if sp > fv: break  # don't pay above BS fair
                qty = min(abs(self.od.sell_orders[sp]),
                          self.soft_cap - self.pos, self.buy_cap)
                if qty > 0: self.buy(sp, qty)
        elif is_sell_bias and self.od.buy_orders:
            # Sweep all bids down to fair value — sell OTM options
            for bp in sorted(self.od.buy_orders.keys(), reverse=True):
                if bp < fv: break  # don't sell below BS fair
                qty = min(abs(self.od.buy_orders[bp]),
                          self.soft_cap + self.pos, self.sell_cap)
                if qty > 0: self.sell(bp, qty)
        else:
            # Neutral MM: take only on large dislocations
