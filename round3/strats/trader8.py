...
HEDGE_PARAMS = {
    ...
    # ---- Direct VEE MM (trader8) ----
    "mm_enabled": True,
    "mm_make_margin": 2,             # quote at vee_ema ± 2 (natural spread = 5)
    "mm_size_frac": 0.15,            # 15% of available budget per side
    "mm_clear_pos": 80,              # |pos| > 80 → flatten at fair
    # ---- VEE range overlay ----
    "range_enabled": True,
    "range_std": 15.0,
...
class HedgeTrader(ProductTrader):
    ...
    def _post_mm(self):
        ...
        # Two-sided make
        bid_price = self.compute_make_bid_price(fair - 0)  # margin captured below
        ask_price = self.compute_make_ask_price(fair + 0)
        # Recompute properly with margin
        # (we override make_margin via direct calc to keep logic explicit)
        fair_bid = fair - margin
        fair_ask = fair + margin
        bb, ba = self.get_best_bid(), self.get_best_ask()
