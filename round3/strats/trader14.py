...
class HedgeTrader(ProductTrader):
    ...
    def _post_mm(self):
        ...
        if self.vee_ema is None:
            return
        # trader14: use current mid as fair (not slow EMA)
        # — quotes track price tightly; range bias derived from mid-vs-EMA dev
        if self.mid is not None:
            fair = round(self.mid)
        else:
        ...
        # Range bias: when mid > EMA, bias toward selling (mean-revert short)
        dev = (self.mid - self.vee_ema) if (self.mid is not None and self.vee_ema is not None) else 0.0
        bias_std = HEDGE_PARAMS["range_std"]
        bid_size_mult = max(0.3, 1.0 - max(0, dev) / bias_std)   # cut bid size when above EMA
        ask_size_mult = max(0.3, 1.0 + max(0, dev) / bias_std)   # boost ask size
        if dev < 0:
            bid_size_mult = max(0.3, 1.0 + abs(dev) / bias_std)
            ask_size_mult = max(0.3, 1.0 - abs(dev) / bias_std)
        ...
        inner_frac = size_frac * 0.5
        outer_frac = size_frac * 0.7

        for m, frac in ((inner_margin, inner_frac), (outer_margin, outer_frac)):
            fair_bid = fair - m
            fair_ask = fair + m
            ...
            if self.max_allowed_buy_volume > 0:
                self.buy(bid_price, int(self.max_allowed_buy_volume * frac * bid_size_mult))
            if self.max_allowed_sell_volume > 0:
                self.sell(ask_price, int(self.max_allowed_sell_volume * frac * ask_size_mult))
