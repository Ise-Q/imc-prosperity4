...
HEDGE_PARAMS = {
    ...
    "max_make_size": 30,
    # ---- Passive hedge band ----
    "hedge_passive_thresh": 5,       # |delta error| <= this → passive limit hedge
    "hedge_market_thresh": 30,       # |delta error| > this → take liquidity
    "passive_step_in_ticks": 1,      # how far inside best to post limit hedge
    # ---- VEE range overlay ----
    "range_enabled": True,
    "range_std": 15.0,
...
class HedgeTrader(ProductTrader):
    ...
    def get_orders(self):
        ...
        # Target = neutralise option delta + lean toward range mean
        ...
        # Below passive_thresh : do nothing this tick
        # passive_thresh < |trade| <= market_thresh : post LIMIT inside best
        # |trade| > market_thresh : walk the book aggressively
        ...
        if abs_trade <= market_thresh:
            # Passive limit hedge: post inside best for slow incremental fills
            if delta_trade > 0:
                px = bb + step
                self.buy(px, delta_trade)
            else:
                px = ba - step
                self.sell(px, -delta_trade)
            return {self.name: self.orders}

        # Aggressive walk : delta is large, walk the book
        if delta_trade > 0:
