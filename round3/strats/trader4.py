...
# trader4 — trader3 regressions corrected + VEE range overlay
#
# Lessons from trader3 backtest (round3 day 0-2 continuous):
#   * HYDROGEL 100% sizing at make_margin=5 inside a 16-tick natural spread
#     produced -66,794 from adverse selection. Rollback to trader2's small
#     sizing and add ONLY a take leg.
#   * vol_tilt on liquid vouchers grew long-call exposure → hedger sold VEE
#     market orders → -12,606 on VEE. Drop vol_tilt entirely.
#   * Liquid voucher MM still warranted, but at very small size with strict
#     inventory skew so the hedger is not stressed.
#
# New: VEE range overlay (user-seeded idea)
#   VEE trades in 5198-5300 (std=15.6). Maintain a wide EMA center;
#   if VEE_mid > center + range_band → lean SHORT residual (sell extra
#   beyond hedge); if mid < center - range_band → lean LONG. Lean amount
#   is bounded so it never violates the position-limit budget after the
#   hedge has been placed.
...
PARAMS = {
    HYDROGEL: {
        ...
        "take_margin": 4,    # tighter take to capture flow
        "clear_margin": 6,
        "make_margin": 6,
        "make_size_frac": 0.1,   # rollback to trader2 size
    },
}
...
HEDGE_PARAMS = {
    ...
    "make_margin": 2,
    "max_make_size": 30,
    "vee_ema_alpha": 0.02,           # slow EMA for range center
    # ---- VEE range overlay ----
    "range_enabled": True,
    "range_std": 15.0,               # EDA std=15.63
    "range_entry_k": 1.0,            # enter when |dev| > k*std
    "range_lean_units": 40,          # max additional inventory to lean
}
...
OPTION_PARAMS = {
    ...
    "default_iv": 0.23,
    "make_skew_per_unit": 2.0,      # strong inventory skew (was 1.0)
    "min_make_half_spread": 1,
    "take_edge": 3.0,
    "make_size_frac": 0.15,         # small MM size on liquid strikes
}
...
# =============================================================================
# HYDROGEL trader — trader2 baseline + a small take leg
# =============================================================================
...
class MeanReversionTrader(ProductTrader):
    def __init__(self, name, state, new_traderData, last_traderData):
        ...
        self.make_size_frac = p.get("make_size_frac", 0.1)
    ...
    def get_orders(self):
        # Take : tight margin to skim flow that crosses fair
...
# =============================================================================
# OptionTrader — deep-ITM intrinsic-take + small symmetric MM on liquid strikes
# =============================================================================
...
class OptionTrader(ProductTrader):
    ...
    def get_orders(self):
        ...
        bid_price = round(bs_fair - half - skew)
        ask_price = round(bs_fair + half - skew)
...
# =============================================================================
# HedgeTrader — VEE: hedge + range overlay
# =============================================================================
...
class HedgeTrader(ProductTrader):
    ...
    def _range_lean(self) -> int:
        """Return signed lean inventory: positive = want to be long, negative = short."""
        ...
        if abs(dev) < thresh:
            return 0
        # Linear lean : capped at ±range_lean_units at |dev|=2*thresh
        lean_units = HEDGE_PARAMS["range_lean_units"]
        scale = min(1.0, (abs(dev) - thresh) / thresh)
        sign = -1 if dev > 0 else +1   # mid above center → want short
        return int(round(sign * scale * lean_units))
