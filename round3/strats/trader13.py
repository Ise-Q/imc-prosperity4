...
class OptionTrader(ProductTrader):
    ...
    def get_orders(self):
        ...
        # ---- trader13: voucher MM with direct book-snap (no double-decrement) ----
        if (OPTION_PARAMS.get("liquid_mm_enabled")
            ...
            if fair > 0:
                ...
                skew = (OPTION_PARAMS["liquid_mm_skew"]
                        * self.expected_position / max(self.position_limit, 1))
                target_bid = fair - half - skew
                target_ask = fair + half - skew
                bb, ba = self.get_best_bid(), self.get_best_ask()
                # Cap our bid at best_ask - 1 (no crossing) and floor at best_bid + 1 (top of book)
                if bb is not None and ba is not None:
                    # Most-aggressive bid we'll post : bb+1 if target_bid > bb, else floor(target_bid)
                    bid_price = int(round(min(target_bid, ba - 1)))
                    if bb is not None and bid_price <= bb:
                        bid_price = bb + 1
                    ask_price = int(round(max(target_ask, bb + 1)))
                    if ba is not None and ask_price >= ba:
                        ask_price = ba - 1
                else:
                    bid_price = int(round(target_bid))
                    ask_price = int(round(target_ask))
...
class HedgeTrader(ProductTrader):
    ...
    def _post_mm(self):
        ...
        # Ladder MM: two levels per side
        # Inner level: tight, more frequent fills
        # Outer level: wider, captures spikes
        bb, ba = self.get_best_bid(), self.get_best_ask()
        inner_margin = margin            # 1 by default
        outer_margin = margin + 2        # 3 by default
