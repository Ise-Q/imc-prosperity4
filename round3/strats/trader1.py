...
# Strats: take mispriced option prices and hedge
# MM HYDDROGEL_PACK using ema, but only post passive quotes with clearing mechanism
...
class MeanReversionTrader(ProductTrader):
    ...
    def get_orders(self):
        ...
        if self.expected_position > 0.5 * self.position_limit:
            self.sell(self.fair_value, max(self.expected_position * 0.1, self.position_limit * 0.05))

        elif self.expected_position < -0.5 * self.position_limit:
            self.buy(self.fair_value, max(self.expected_position * 0.1, self.position_limit * 0.05))

        return {self.name: self.orders}

class HedgeTrader(ProductTrader):
    
    def __init__(self, name, state, new_traderData, OptionTrades):
        super().init(name, state, new_traderData)
        

# =============================================================================
# OptionTrader — lifts asks < max(S-K,0), hits bids > S
# =============================================================================
